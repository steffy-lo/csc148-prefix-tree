...
class SimplePrefixTree(Autocompleter):
            ...
            if self.subtrees != []:
                ...
                for subtree in self.subtrees:
                    if subtree.value == value:
                        self.update_weight(weight, False)
                        subtree.update_weight(weight, False)
                        insert_leaf = False
    ...
    def update_weight(self, weight: float, mutated: Optional[bool] = True) \
            -> None:
        if self.weight_type == 'sum':
            ...
            if len(self) <= 1:
                ...
                if mutated is True:
                    ...
                    self.weight = \
                        (len(self) * self.weight + weight) / len(self)

    def insert_new_leaf(self, value: Any, weight: float) -> None:
            ...
            if limit is None:
                for subtree in self.subtrees:
                    if self.value == prefix[:len(self.value)] or \
                        ...
                        if subtree.subtrees != [] or len(self.value) >= \
                                len(prefix):
...
class CompressedPrefixTree(SimplePrefixTree):
    ...
    def insert(self, value: Any, weight: float, prefix: List) -> None:
                    ...
                    if prefix[:i] == subtree.value[:i]:
                        ...
                        if len(prefix) == len(subtree.value):
                            if prefix == subtree.value:
                                self.insert_update_leaf(subtree, value, weight)
                            else:
                                new_tree = \
                                    CompressedPrefixTree(self.weight_type)
                                new_tree.value = prefix[:i]
                                new_tree.subtrees.append(self.new_tree_leaf
                                                         (value,
                                                          weight,
                                                          prefix))
                                if new_tree.weight_type == 'sum':
                            ...
                            # len(prefix) cannot be <= subtree.value, since the
                            # first if condition will go through
                            if self.weight_type == 'sum':
                                self.weight += weight
                            else:
                                self.weight = \
        ...
        if len(new_old_subtree) >= 2:
            exist = False
            if self.value == new_old_subtree[0].value:
                self.subtrees.append(self.new_tree_leaf(value, weight, prefix))
                self.update_weight(weight)
                exist = True
            if exist is False:
    ...
    def insert_update_leaf(self, tree: CompressedPrefixTree,
        ...
        for subtree in tree.subtrees:
            if subtree.value == value:
                inserted = True
                self.update_weight(weight, False)
                tree.update_weight(weight, False)
                subtree.update_weight(weight)
        if inserted is False:
            tree.insert_new_leaf(value, weight)
            self.update_weight(weight)
            tree.update_weight(weight)
            tree.subtrees.sort(key=lambda trees: trees.weight, reverse=True)

    def new_tree_leaf(self, value: Any, weight: float, prefix: List) \
