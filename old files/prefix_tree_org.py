...
class SimplePrefixTree(Autocompleter):
    ...
    def __len__(self) -> int:
        ...
        if self.is_empty():
            return 0
        elif self.subtrees == [] and self.weight > 0:  # this tree is a leaf
            return 1
        else:
            num = 0
    ...
    def insert(self, value: Any, weight: float, prefix: List) -> None:
        ...
        if prefix == []:
            if self.subtrees != []:
                insert_leaf = True
                for subtree in self.subtrees:
                    if subtree.subtrees == [] and weight > 0:  # if leaf exists
        ...
        if self.is_empty():
                    ...
                    if prefix[:i] == subtree.value[:i]:
                        ...
                        if i < len(subtree.value):
                            if len(prefix) >= len(subtree.value):
                                ...
                                new_tree = self.new_tree_leaf(value,
                                                              weight,
                                                              prefix)
                                new_old_subtree.append(new_tree)
                                new_old_subtree.append(subtree)
                            ...
                            #     if isinstance(subsubtree.value, list):
                            #         if prefix[:i + 1] == subsubtree.value[
                            #                              :i + 1]:
                            #             subtree.insert(value, weight, prefix)
                            if len(subtree.subtrees) == 1:
                                # if value != subtree.subtrees[0]:
                                #     # this subtree is a parent of a leaf,
                                #     # and the value we are inserting is not
                                #     # the same
                                #     subtree.subtrees.append(self.new_tree_leaf(
                                #         value, weight, prefix))
                                #     self.update_weight(weight)
                                #     subtree.update_weight(weight)
                                # the value inserted will be the same
                                    self.update_weight(weight)
                                    subtree.update_weight(weight)
                                    subtree.subtrees[0].update_weight(weight)
                            else:
                                self.weight = (len(self)*self.weight +
                                               weight) / (len(self) + 1)
                                subtree.insert(value, weight, prefix)
            ...
            if insert is False:
                self.subtrees.append(self.new_tree_leaf(value,
                                                        weight,
                                                        prefix))
                # print(self.value, self.weight, len(self))
                self.update_weight(weight)
                # print(self.value, self.weight, len(self))
                self.subtrees.sort(key=lambda trees: trees.weight, reverse=True)
